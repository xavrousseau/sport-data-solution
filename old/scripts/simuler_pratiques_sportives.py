...
# Liste d'activités enrichie (déplacements + sport personnel)
ACTIVITES = [
...
# ==========================================================================================
def simuler_activites_strava(df_salaries, nb_mois, activites_min, activites_max):
    activities = []
    ...
    for idx, row in df_salaries.iterrows():
        ...
        for _ in range(nb_activites):
            ...
            date_activite = date_debut + timedelta(days=randint(0, nb_mois * 30), hours=randint(6, 20), minutes=randint(0, 59))
            ...
            activities.append({
                ...
                "date_debut": date_activite.strftime("%Y-%m-%d %H:%M"),
    ...
    df_activites = pd.DataFrame(activities)
    logger.info(f"Nombre total d'activités simulées : {len(df_activites)}")
    return df_activites

# ==========================================================================================
# 5. Export Excel et PostgreSQL
# ==========================================================================================
...
if __name__ == "__main__":
    try:
        ...
        df_activites = simuler_activites_strava(df_salaries, NB_MOIS, ACTIVITES_MIN, ACTIVITES_MAX)
                # === Récapitulatif des activités simulées ===
        ...
        repartition = df_activites["sport_type"].value_counts()
        logger.info("\n🏅 Répartition par activité :")
        for sport, count in repartition.items():
            logger.info(f" - {sport:<15} : {count} activité(s)")

        # Top 5 des salariés les plus actifs
        top_salaries = df_activites["id_salarie"].value_counts().head(5)
        logger.info("\n👥 Top 5 des salariés les plus actifs :")
        for id_salarie, count in top_salaries.items():
        ...
        logger.info("=== Pipeline terminé : PostgreSQL + MinIO + ntfy ✅ ===")
    except Exception as e:
